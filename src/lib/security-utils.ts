...
export function encrypt(text: string): string {
    ...
    const cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(ENCRYPTION_KEY).subarray(0, 32), iv)
...
export function decrypt(text: string): string {
    ...
    const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(ENCRYPTION_KEY).subarray(0, 32), iv)
