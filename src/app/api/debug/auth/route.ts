...
export async function GET() {
  ...
  return NextResponse.json({
    env: {
      ...
      GOOGLE_CLIENT_ID: googleId     ? `${googleId.substring(0, 8)}...` : '⚠️ NOT SET (Google OAuth disabled)',
      GOOGLE_SECRET:    googleSecret ? 'SET (hidden)'                   : '⚠️ NOT SET (Google OAuth disabled)',
    ...
    callbackUrls: {
      github: process.env.NEXTAUTH_URL
        ? `${process.env.NEXTAUTH_URL}/api/auth/callback/github`
        : '❌ NEXTAUTH_URL not set',
      google: process.env.NEXTAUTH_URL
        ? `${process.env.NEXTAUTH_URL}/api/auth/callback/google`
        : '❌ NEXTAUTH_URL not set',
    },
    warnings: [
      !githubId         && '❌ GITHUB_ID not set — GitHub login broken',
      !githubSecret     && '❌ GITHUB_SECRET not set — GitHub login broken',
      !googleId         && '⚠️ GOOGLE_CLIENT_ID not set — Google button hidden',
      !googleSecret     && '⚠️ GOOGLE_CLIENT_SECRET not set — Google button hidden',
