...
async function main() {
  ...
  if (databaseUrl?.includes('HOST:5432') || databaseUrl?.includes('HOST/')) {
    writePendingReport(
      target,
      'La DATABASE_URL parece tener placeholders (`HOST`) y no una URL real de base de datos.',
      'Reemplazar HOST/USER/PASS/DB por valores reales del proveedor (Neon/Railway/etc).'
    )
  ...
  if (!databaseUrl) {
    writePendingReport(target, 'No se encontró la variable de entorno `DATABASE_URL` en este entorno local.')
    console.log(`KPI baseline pending report generated at: ${target}`)
    return
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Error desconocido al conectar con base de datos'
    writePendingReport(target, `No se pudo conectar a la base de datos: ${message}`)
    console.log(`KPI baseline pending report generated at: ${target}`)
  }
